#!/usr/bin/env python3
"""Flask dashboard for polymarket_5m_eth_trader — serves at 0.0.0.0:8098."""
...
LOG_PATH = Path(os.getenv("POLYMARKET_ETH_TRADER_LOG", str(APP_DIR / "polymarket_5m_eth_trader.log")))
TRADES_CSV = Path(os.getenv("POLYMARKET_ETH_TRADER_TRADES_CSV", str(APP_DIR / "polymarket_5m_eth_trader_trades.csv")))
PORTFOLIO_CSV = Path(os.getenv("POLYMARKET_ETH_TRADER_PORTFOLIO_CSV", str(APP_DIR / "polymarket_5m_eth_trader_portfolio.csv")))
...
def up_rate_24h() -> dict[str, Any]:
    """Compute Up% from outcome rows in the last 24 h of trading data."""
    rows = _read_csv_rows(TRADES_CSV)
...
<title>Polymarket ETH 5m Trader</title>
<style>
  ...
  <div class="sidebar">
    ...
    <div class="card">
      ...
      <div class="metric-grid">
        ...
        <div class="metric-item" style="grid-column:1/-1;">
          <div class="label" style="font-size:9px;color:#404070;">Backtest mean: 50.4% · balanced ±2.4pp/day · warning if >55% or <45%</div>
        </div>
      </div>
    </div>
...
function updateUpRate(data) {
  ...
  // color: green if near 50%, yellow if 45–55%, red if trending hard
...
if __name__ == "__main__":
    ...
    a = p.parse_args()
    print(f"Starting Polymarket ETH 5m trader dashboard at http://{a.host}:{a.port}")
