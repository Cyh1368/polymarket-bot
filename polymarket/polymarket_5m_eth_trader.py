...
Entry rule (2026-06-18 research, saved_huber_model_eth_t265, min_child=150):
  ...
  14 features: v3 base (13) + obi_depth_slope. Note: at T=265s the ETH market
    has been open ~35s so _60 windows are truncated to ~15s of actual data.
    The model was trained on the same truncated windows — consistent.
  50-seed CV: EV/available=+3.72%, win-capped=+5.95%, trim10=+7.60%,
    CI lo +3.38%, 50/50 seeds pass gate.
  Model files: polymarket/saved_huber_model_eth_t265/huber_yes_t265.txt,
    huber_no_t265.txt

No exit: hold to official settlement outcome.

Usage:
  python polymarket_5m_eth_trader.py                   # dry run
  python polymarket_5m_eth_trader.py --live            # live trading
...
MODEL_YES_PATH = Path(__file__).resolve().parent / "saved_huber_model_eth_t265" / "huber_yes_t265.txt"
MODEL_NO_PATH  = Path(__file__).resolve().parent / "saved_huber_model_eth_t265" / "huber_no_t265.txt"
...
DEFAULT_DATA_DIR = APP_DIR / "data_ETH_5m"
DEFAULT_OUTCOMES_CSV = APP_DIR / "polymarket_eth_5m_official_outcomes.csv"
LOG_PATH = Path(
    os.getenv("POLYMARKET_ETH_TRADER_LOG", str(APP_DIR / "polymarket_5m_eth_trader.log"))
)
TRADES_CSV_PATH = Path(
    os.getenv("POLYMARKET_ETH_TRADER_TRADES_CSV", str(APP_DIR / "polymarket_5m_eth_trader_trades.csv"))
)
PORTFOLIO_CSV_PATH = Path(
    os.getenv("POLYMARKET_ETH_TRADER_PORTFOLIO_CSV", str(APP_DIR / "polymarket_5m_eth_trader_portfolio.csv"))
)
...
async def discover_current_market(client: httpx.AsyncClient) -> CurrentMarket:
    ...
    for slot in slots:
        ...
        if market:
            return market
    raise RuntimeError("No active ETH 5m Up/Down market found on Polymarket")
...
async def run(args: argparse.Namespace) -> None:
    ...
    for p in (MODEL_YES_PATH, MODEL_NO_PATH):
        if not p.exists():
            raise RuntimeError(f"Model file not found: {p}  — run 2026-06-17-research/train_eth_t265.py to generate ETH models")
    ...
    append_log(
        ...
        f"model=huber_edge_eth_t265[yes,no] skip_bonus={SKIP_BONUS} filters=none "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Polymarket ETH 5m Up/Down live trader — Huber edge-regression model at T=265s.")
