#!/usr/bin/env python3
"""
polymarket/train_lgb_v3p1.py

Train and save the production LightGBM v3.1 binary classifier.
v3.1 = v3 (13 features) + obi_depth_slope (14th feature, NaN for pre-tau contracts).

Trains on ALL available BTC 5m contracts at T1=180s — no held-out split.
Output: polymarket/lgb_v3p1_t180.txt  (LightGBM native text format)

obi_depth_slope: OLS slope of up_book_imbalance_tau_{X}c vs log(X/100) across
8 tau levels (1c,2c,3c,5c,7c,10c,15c,20c). NaN for pre-tau contracts (<2 valid
levels). LightGBM routes NaN to the optimal branch at each split.

CV result (2026-06-15): v3.1 EV/avail=+5.45% vs v3 +2.80% (4-fold expanding CV,
skip_bonus=0.05, Filter B). Paired Δ=+1.83pp CI=[+1.40%,+2.25%] (200-seed).
"""
...
MODEL_OUT     = APP_DIR / "lgb_v3p1_t180.txt"
...
FEATURES = V3_FEATURES + ["obi_depth_slope"]
...
def ols_slope(y_vals: np.ndarray) -> float:
    """OLS slope of y_vals vs TAU_LOG_X. Returns NaN when <2 valid points."""
...
def extract_row(cd: ContractData) -> dict | None:
    ...
    if ts_ok:
        h60 = df[df["_ts"].notna() & (df["_ts"] <= t1_ts) &
                 ((t1_ts - df["_ts"]).dt.total_seconds() <= 60.0)]
    ...
    slope    = ols_slope(tau_vals)   # NaN for pre-tau contracts
    ...
    # Only require V3_FEATURES to be finite; obi_depth_slope may be NaN
...
def main():
    ...
    tau_n = int(df["obi_depth_slope"].notna().sum())
    print(f"  {len(df)} rows at T1={T1}s  "
          f"UP={int(df['y_settle'].sum())}  DOWN={int((df['y_settle']==0).sum())}")
    print(f"  tau_present={tau_n} ({tau_n/len(df)*100:.1f}%)  tau_nan={len(df)-tau_n}")

    X = df[FEATURES].to_numpy().astype(float)   # NaN values left as NaN
    ...
    print("Feature importance (gain, top 8):")
    for feat, gain in fi[:8]:
        print(f"  {feat:22s}  {gain:.0f}")
